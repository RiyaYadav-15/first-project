  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div 
        ...
        className="relative w-full max-w-lg bg-[#242220] rounded-xl shadow-2xl border border-[#383430] overflow-hidden my-6"
      >
        {/* Header */}
        ...
        <div className="bg-[#1a1816] p-4 border-b border-[#383430] flex items-center gap-3">
          <img
            src={mentor.avatarUrl}
            alt={mentor.fullName}
            className="w-12 h-12 rounded-full object-cover border border-[#383430]"
          />
          <div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {mentor.mentorshipTopics && mentor.mentorshipTopics.length > 0 && (
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-[#c99c7b] mb-1.5">
                ...
                {mentor.mentorshipTopics.map((t, idx) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded text-xs transition-colors cursor-pointer text-left ${
                      topic === t 
                        ? 'bg-[#8d7d70] text-[#141414] font-bold' 
                        : 'bg-[#1a1816] hover:bg-[#2d2a27] text-[#d6cfc7] border border-[#383430]'
                    }`}
                  >
                    {t}
                  </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                <Calendar className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-[#8d7d70]" />
                <input
                  id="booking-date"
                  type="date"
                  value={preferredDate}
                  onChange={(e) => setPreferredDate(e.target.value)}
                  className="w-full pl-8 pr-2.5 py-1.5 text-xs bg-[#1a1816] border border-[#383430] rounded-md text-[#f5f2ed] focus:outline-none focus:border-[#8d7d70]"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Clock className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-[#8d7d70]" />
                <input
                  id="booking-time"
                  type="text"
                  value={sessionTime}
          ...
          <div className="p-3 bg-[#1c1a18] rounded-lg border border-[#383430] text-[11px] text-[#a1958b] flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-[#8d7d70] shrink-0" />
            <span>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              <Send className="w-3.5 h-3.5 text-[#141414]" />
